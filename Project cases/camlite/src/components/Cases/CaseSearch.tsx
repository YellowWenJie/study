...
export default function CaseSearch(props: { user: User }) {
  const { user } = props;
  ...
  function saveQuery(event: FormEvent) {
    ...
    let caseQuery = {
      usersId: user.id,
      title: 'Case',
      urlHistory: link.caseSearch,
      params: `${link.caseSearchResult}?${params}`,
    };
  ...
  function handleSubmit(event: FormEvent) {
    ...
    let params = new URLSearchParams(obj).toString();
    if (params) {
      history.push(`${link.caseSearch}?${params}`);
    }
  }
    ...
    <form onSubmit={handleSubmit}>
      <div className={'govuk-grid-row'}>
        <div className="govuk-grid-column-full">
          <h1 className="govuk-heading-l">Search Cases</h1>
        </div>
